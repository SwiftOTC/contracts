...
contract SwiftOTCPrivate is ReentrancyGuard, Ownable {
    struct Order {
        uint256 orderId;
        address seller;
        address buyer;
   ...
   function createOrder(
    ...
    require(_allowedBuyer != address(0), "Allowed buyer address cannot be zero");
    require(
...
function acceptOrder(uint256 _orderId) public nonReentrant {
    ...
    require(orders[_orderId].allowedBuyer == msg.sender, "Caller is not the allowed buyer");
    require(
    ...
    IERC20(orders[_orderId].tokenToSell).transfer(
